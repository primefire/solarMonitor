import PowerReadingModel from './Database/Models/PowerReadingModel';
...
export default class PowerUpdateHandler {
	...
	private saveAveragesAndTimestampInDatabase(solarAvg: number, gridAvg: number): void {
		let databaseModel = new PowerReadingModel({ solar: solarAvg, grid: gridAvg, timestamp: new Date(this.latestTimestampString) });
		databaseModel.save();
	}
